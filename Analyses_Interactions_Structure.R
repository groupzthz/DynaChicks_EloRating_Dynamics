...
library(moments) # normal assumptions
...
library(binom) #confidence interval for binomial sample
...
#exclusion of accidentally double tagged ID
...
# ATTENTION!: The Focus, Position & Strategy calculations are veeeery slow for
# the large groups
...
#save data for analyses of other tests
fwrite(Interact, "IndividualsOutput.csv", sep = ";")


# ratio of interactions and individual was involved in compared to the total interactions
Interact[, ratio := sum/(sum(sum)*0.5), by = Pen]

#exclude individuals without Elo from analysis
Interact = Interact[!is.na(elos),]

####

#Data.table containing:
#Every single Interaction with updated ranks and elos of the involved individuals
...
# incidence rate per bird per hour
...
# poly(raw= T) otherwise uninterpretable estimates with raw = F 
...
resid.sum<- simulateResiduals(sum.model, 1000)
plot(resid.sum) #overdispersal!! -> negative binomial model necessary
...
resid.sum<- simulateResiduals(sum.model, 1000)
plot(resid.sum) 
plotResiduals(resid.sum, form = InteractWide$Group_Size) #good
plotResiduals(resid.sum, form = InteractWide$Situation) #problematic? no homoscedacity not assumed in negative binomial
plotResiduals(resid.sum, form = InteractWide$scaleElos) #good

sum.model.null = glmer.nb(Sum ~ 1 +(1|Pen), InteractWide) 
anova(sum.model, sum.model.null) #better than intercept only model
...
#reduced final model
sum.model.red = glmer.nb(Sum ~ poly(scaleElos,2, raw = TRUE)+Group_Size +Situation+offset(log(Minutes))+(1|Pen), InteractWide)
resid.sum<- simulateResiduals(sum.model.red, 1000)
plot(resid.sum)
plotResiduals(resid.sum, form = InteractWide$Group_Size) #good
plotResiduals(resid.sum, form = InteractWide$Situation) #problematic? no homoscedacity not assumed in negative binomial
plotResiduals(resid.sum, form = InteractWide$scaleElos) # good
AIC(sum.model.red, sum.model.null)

#model estimate
summary(sum.model.red)
parameters(sum.model.red, exp = TRUE)
tab_model(sum.model.red)
plot(predictorEffects(sum.model.red), lines=list(multiline=TRUE))
summary(allEffects(sum.model.red))

#estimate variance explained
r.squaredGLMM(sum.model.red, sum.model.null)

#Post-hoc comparison of conditions
postHocCond = emmeans(sum.model.red, ~ pairwise ~Situation, 
...
InteractWide[, PredictSum := (predict(sum.model.red, type = "response"))]
...
plotData = as.data.table(emmeans(sum.model.red, ~ pairwise ~Situation*Group_Size, type = "response", offset = log(InteractWide$Minutes))$emmeans)
...
plotData <- as.data.table(emmeans(sum.model.red, ~ pairwise ~Situation, type = "response", offset = log(InteractWide$Minutes))$emmeans)

#plot used in paper
#Sum of interactions by resource condition
condPlot = 
  ggplot(plotData, aes(x = Situation, y = response))+
...
#descriptors
descript = RankTable[, .(Sum = sum(AggressBool), N = .N, 
                         perc = sum(AggressBool)/.N), by = Pen]


##### statistical model ########################################################

#to change level orders for plotting consistency
...
intensity.model.red = glmer(AggressBool ~ WinnerElo+ LoserElo + Situation+ Group_Size + 
                                         Group_Size:Situation + LoserElo:WinnerElo + (1|Dyad), 
                        data = RankTable, family = binomial, 
                        glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)))
resid.intensity = simulateResiduals(intensity.model.red)
plot(resid.intensity)
plotResiduals(resid.intensity, form = RankTable$DiffElo) #nearly perfect
plotResiduals(resid.intensity, form = RankTable$Group_Size) #good
plotResiduals(resid.intensity, form = RankTable$Situation) #good

anova(intensity.model.red, intensity.null) #better than null model

#model estimates
summary(intensity.model.red)
parameters(intensity.model.red, exponentiate = TRUE)
tab_model(intensity.model.red)
#plot(predictorEffects(intensity.model.red), lines=list(multiline=TRUE))
plot(allEffects(intensity.model.red))
plot_model(intensity.model.red, show.value = T)

#explained variance
r.squaredGLMM(intensity.model.red, intensity.null)

#estimated means and post hoc testing

#aggression intensity by winnerelo fixed at min, mean, or max loserelo
emmeans(intensity.model.red, ~ WinnerElo, 
        at = list(WinnerElo = c(min(RankTable$WinnerElo),mean(RankTable$WinnerElo),max(RankTable$WinnerElo))), type= "response")

winBeta = summary(emtrends(intensity.model.red, ~ LoserElo, var="WinnerElo", 
...
group_sit = emmeans(intensity.model.red, ~ pairwise ~ Group_Size*Situation, type = "response")
...
plotData = as.data.table(summary(emmeans(intensity.model.red, ~ WinnerElo*LoserElo, 
...
#plot used in paper
# shows probability for high intensity aggression with respect to the Elo rating
intensPlot1 = 
...
plotData <- as.data.table(emmeans(intensity.model.red, ~ pairwise ~Situation*Group_Size, type = "response")$emmeans)
...
#plot used in paper
# Probability for high intensity aggression by resource condition and group size
...
# Supplementary plots
#using function from helper_functions
...
#arranging and saving the figure
...
ggplot(data = Interact, mapping = aes(x = scaleElos, y =Contacts, colour = Pen)) + 
  geom_smooth(se= FALSE)+#method = lm, formula = y ~ splines::bs(x, 3), se = FALSE)+
  geom_point(size = 2.5) + 
  labs(x = 'Elo rating', y = 'Sum of unique interactions')+
  theme_classic(base_size = 18)
...
#bringing both plots together in one figure
